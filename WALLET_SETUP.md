...
The existing Profit Tracker, Cashout Reports, reviewer access and INFI review are retained. The new wallet is under Profit & Wallet; Payout Calculator opens its Team payouts view. Previous Payouts keeps the prior calculator available during migration. Legacy cashout metadata embedded in client_paid_date is preserved by the SQL function. Use the new wallet for new ledger entries; old profit reports are not automatically reconciled into the new ledger. Review client-specific billing and artist rates before importing new wallet snapshots.
